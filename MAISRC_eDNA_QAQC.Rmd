...
qPCR_LOD_Calc from https://github.com/cmerkes/qPCR_LOD_Calc

Citation to use:
Merkes CM, Klymus KE, Allison MJ, Goldberg C, Helbing CC, Hunter ME, Jackson CA, Lance RF, Mangan AM, Monroe EM, Piaggio AJ, Stokdyk JP, Wilson CC, Richter C. (2019) Generic qPCR Limit of Detection (LOD) / Limit of Quantification (LOQ) calculator. R Script. Available at: https://github.com/cmerkes/qPCR_LOD_Calc. DOI: https://doi.org/10.5066/P9GT00GB. Date Accessed:05/09/2022

This script comes from the above website/citation. I (Adelle Keppers) made minimal changes to this script so that it would work the way I intend in the data wrangling section (below). I marked my changes with my initials (ACK) towards the begin of a commented line or after a script line by adding a comment. 

```{r}
# ACK = Make script into a function so I can easily pass in the data I want. Once this function is in your global enviornment you will not have to run this section of the script again, even if you add new data, unless you change the function of the script itself.

lod_loq_calc <- function(DAT) { # ACK = start of function
...
# ACK = Already done in Setup section (above)
## Load packages:
#library(ggplot2)
#library(drc)

# ACK = Already done in Setup section (above)
## ACK ="Line 17" = Set your working directory to where your csv file is saved (MODIFY AS NEEDED): 
#setwd("G:/My Drive/2021 MAISRC eDNA Project/data_analysis")

# ACK = Already done in setup section
## ACK = "Line 20" = Read in your data file (MODIFY FILE NAME AS NEEDED):
#ACK = commented out this line "DAT <- read.csv("test_lod_loq_data.csv")" Don't need it anymore.

## ACK = "Line 23" = Define your CV threshold for LoQ:
LOQ.Threshold <- 0.35

## ACK = "Line 26" = Define which logarithmic function to use for LoD model:
LOD.FCT <- "Best"
...
## ACK = Line "33" = Define which model to use for LoQ model:
LOQ.FCT <- "Best"
...
write.csv(DAT3,paste0(format(Sys.time(), "%d-%b-%Y %H.%M"),".csv"))
} # ACK = close of function 

# ACK = Below are all plots this script can generate. Turn on and Off as needed.
...
Load files and do some data wrangling so that it is in a useful format.
Requirements for this function to work:

  After a plate is run do the following:
  1- Perform all QC as stated in qPCR_activity_one_results_quide.
  2- Export file (.xls) from StepOne Plus Software V2.3. 
        Make sure file contains only the following columns in the following order. This can be changed in the export settings.
              Sample Name
              Target Name
              Task
              CT
              ...
              Quantity Mean
              Quantity SD
  3- Make sure the script below has the correct path to find your exported files
  4- Run the script. 

```{r, warning = FALSE}
filenames <- dir(path = "./data/pcr_data", pattern = ".xls") # Getting all .xls filenames from current working directory
results <- list() # Initializing an empty list

for (filename in filenames) {
  # Read StepOne Plus excel file
  a <- read_xls(paste0("./data/pcr_data/", filename))
  
  # Rename columns. target, cq and sq should not be changed. These names are needed for the LOD/LOQ script to run.
  colnames(a) <- c("uid", "target", "task", "cq", "ct_mean", 
                   "ct_sd", "sq", "sq_mean", "sq_sd")
  
  # Remove unwanted data
  a = a[8:103,]  # Remove unwanted rows (top rows with file name, time or run, etc.)
  
  # Add presence absence column where presence = detection (cq) of any value, absence = not detection value (undetermined)
  a <- a %>% 
    mutate(pres_abs = ifelse(a$cq == "Undetermined", "0", "1")) 
  
  # Change vector type of all columns from "character" to appropriate type.
  a$cq <- as.numeric(a$cq) # You can ignore this warning. This will replace "undetermined" with "NA". This is acceptable as no data is lost and interpretation is the same. 
  a$ct_mean <- as.numeric(a$ct_mean)
  a$ct_sd <- as.numeric(a$ct_sd)
  a$sq <- as.numeric(a$sq)
  a$sq_mean <- as.numeric(a$sq_mean)
  a$sq_sd <- as.numeric(a$sq_sd)
  a$pres_abs <- as.numeric(a$pres_abs)
  
  ## Add LOD and LOQ columns ##
  
  # Fist make a data frame of only the data needed to run the LOD/LOQ function
  #b <- as.data.frame(a %>% # make data from from object "a"
  #      filter(task == "STANDARD")) # only add in standard rows
  #b = b[ , c(2,4,7)] # remove unwanted column (keep target, cq and sq)
  
  # Pass object "b" into the LOD_LOQ function/script and calculate the LOD_LOQ for this data set
  #lod_loq_calc(b)
  
  # Pull results from LOD_LOQ function/script and add back into object "a" as new columns
  #a = cbind(a, LOD = DAT3$LOD)
  #a = cbind(a, LOQ = DAT3$LOQ)
  
  # Rename file and add to empty list
  results[[filename]] <- a  
  
  # Clean up temporary files used for data wrangling that are no longer needed
  rm("a")
  #rm("b")
}
```
*** Need to add in replicate column still. Will add that in the above section when I have time. For now I did it manually as there are only a few data sets. This will get more difficult the more data that is added.~~~~


Merge all data generated from the data wrangling into one data frame. This will make it easier to manipulate the data for downstream analysis.

```{r df}
# Make empty dataframe
df_eDNA = as.data.frame(results[[1]][FALSE,])
for (filename in results) {
  # Append data from all results to data frame.
  df_eDNA <- bind_rows(df_eDNA, filename)
}
#view(df_eDNA) # visually confirm these steps
```


Filter out only the data that we want. Remove the standards and plate blanks.
```{r rmstandards_blanks}
df_eDNA <- df_eDNA %>%
  filter(task == "UNKNOWN", uid != "NA") #Remove standards and plate blanks
df_eDNA = df_eDNA[ , c(1,2,7:10)] #Remove unwanted columns
...
```{r writecsv}
write.csv(df_eDNA, 
          paste0("./data/", format(Sys.time(), "%d-%b-%Y"),
                 "_qPCR_activity_one_results.csv"),
          row.names = FALSE)
```
