...
```{r, warning = FALSE}
filenames <- dir(path = "./data/pcr_data1", pattern = ".xls") # Getting all .xls filenames from current working directory
results <- list() # Initializing an empty list

for (filename in filenames) {
  # Read StepOne Plus excel file
  a <- suppressMessages(read_xls(paste0("./data/pcr_data1/", filename)))
  ...
  a <- a %>% mutate(pres_abs = ifelse(a$cq == "Undetermined", "0", "1")) 
...
df_eDNA$replicate <- rep(c("pcr_1", "pcr_2", "pcr_3"))
...
```{r writecsv}
write.csv(df_eDNA,paste0("./data/", format(Sys.time(), "%d-%b-%Y"),
                 "_qPCR_activity_one_results.csv"), row.names = FALSE)
```
